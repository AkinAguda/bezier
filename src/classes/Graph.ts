...
export default class Graph {
  ...
  constructor(proprties: GraphInterfaces) {
    ...
    this._canvas.addEventListener("touchstart", (event) => {
      ...
      const rect = this._canvas.getBoundingClientRect();
      this.drag.point = this.pointRaw(
        event.changedTouches[(event as TouchEvent).changedTouches.length - 1]
          .clientX - rect.left,
    ...
    this._canvas.addEventListener("touchmove", (event) =>
      this.handleTouchMove(event, proprties)
    );
    ...
    this._canvas.addEventListener("dblclick", (event) => {
      ...
      proprties.points.state.forEach((controlPoint) => {
        if (
          distBetweenPoints(controlPoint, this.cursor) <= controlPoint.radius
        ) {
